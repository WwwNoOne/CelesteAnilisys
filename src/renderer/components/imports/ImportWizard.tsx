...
import { analyzeImport, approveImport, ensureCompany, ensurePeriod, fileFromBase64, getImportIssues, getImportSheets, getSheetPreview, reviewImportRow, updateImportPeriod, uploadImport, type ImportIssue, type ImportPreview, type ImportResult, type ImportSheet } from '../../api';
...
export function ImportWizard({ company, onClose, onOpenReview }: ImportWizardProps) {
  ...
  async function analyze() {
    ...
    try {
      ...
      const uploadPeriod = String(new Date().getFullYear());
      const backendPeriod = await ensurePeriod(backendCompany.id, uploadPeriod);
  ...
  async function validatePeriod() {
    if (!result || !period.trim()) return;
    const year = Number.parseInt(period, 10);
    if (!Number.isInteger(year)) { setError('El período debe ser un año válido'); return; }
    try { setResult(await updateImportPeriod(result.id, period.trim(), year)); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'No se pudo validar el período'); }
  }
  ...
  return <div className={`modal-backdrop ${result ? 'review-backdrop' : ''}`} role="presentation"><section className={`import-modal ${result ? 'review-modal' : ''}`} role="dialog" aria-modal="true" aria-labelledby="import-title">
    ...
    {!result && <><button className="dropzone" type="button" onClick={chooseFiles}><span className="upload-icon small">↑</span><strong>Arrastra aquí tu archivo Excel</strong><span>o haz clic para seleccionar · .xlsx, .xlsm</span></button><input ref={inputRef} className="visually-hidden" type="file" accept=".xlsx,.xlsm" multiple onChange={handleBrowserSelection} />{selectedFiles.length > 0 && <div className="selected-files">{selectedFiles.map((file) => <span key={file}>✓ {fileNameFromPath(file)}</span>)}</div>}</>}
    {error && <p className="form-error" role="alert">{error}</p>}
    {result && <div className="import-review"><div className="period-review"><label className="field-label">Período detectado <input value={period} placeholder="Ej. 2025" onChange={(event) => setPeriod(event.target.value)} /></label><button className="secondary-button" type="button" disabled={!period.trim() || result.period_validated} onClick={() => void validatePeriod()}>{result.period_validated ? 'Período validado' : 'Validar período'}</button><small>{result.period_source ? `Origen: ${result.period_source}` : 'No detectado automáticamente'}</small></div><div className="import-result"><strong>{result.status === 'APPROVED' ? 'Importación aprobada' : 'Archivo analizado correctamente'}</strong><span>{result.total_rows} filas detectadas · {result.recognized_rows} reconocidas · {result.review_rows} requieren revisión</span></div><strong className="review-title">Hojas detectadas</strong><div className="sheet-list">{sheets.map((sheet) => <button className={selectedSheet === sheet.sheet_name ? 'sheet-selected' : ''} key={sheet.sheet_name} type="button" onClick={() => void selectSheet(sheet.sheet_name)}>✓ {sheet.sheet_name} <small>{sheet.sheet_type}</small></button>)}</div>{preview && <><strong className="review-title">Vista previa: {preview.sheet_name}</strong><div className="preview-table-wrap"><table className="preview-table"><tbody>{preview.rows.map((row, rowIndex) => <tr key={rowIndex}>{row.map((value, columnIndex) => <td key={`${rowIndex}-${columnIndex}`}>{value ?? ''}</td>)}</tr>)}</tbody></table></div></>}{issues.length > 0 && <><strong className="review-title warning">Filas para revisión</strong><div className="issue-list">{issues.slice(0, 5).map((issue) => <span key={issue.id}>{issue.source_sheet} · fila {issue.source_row} · {issue.original_name ?? 'Sin nombre'} <small>{issue.status}</small><button type="button" onClick={() => void ignoreRow(issue.id)}>Ignorar</button></span>)}</div></>}</div>}
    <div className="modal-actions">
      <button className="secondary-button" type="button" onClick={onClose}>Cerrar</button>
      {result && onOpenReview && (
        <button className="secondary-button" type="button" onClick={() => onOpenReview(result.id)}>
          Abrir en espacio completo ↗
        </button>
      )}
      {result ? <button className="primary-button" type="button" disabled={issues.length > 0 || !result.period_validated || result.status === 'APPROVED'} onClick={() => void approve()}>{result.status === 'APPROVED' ? 'Aprobada' : 'Aprobar importación'}</button> : <button className="primary-button" type="button" disabled={!selectedFiles.length || loading} onClick={() => void analyze()}>{loading ? 'Analizando…' : 'Analizar archivo'}</button>}
    </div>
  </section></div>;
}
